...
var scheme = {
  ...
  buildMessageToSign: function(req, encoding) {
    ...
    var CanonicalRequest =
        ...
        this._hash(body, encoding.output || 'hex', encoding.input);

    if (this.config.debug) {
      this._lastCanonicalRequest = CanonicalRequest;
      this._lastRequestBody = body;
      this.log(CanonicalRequest);
    }
  ...
  sign: function(req, credentials, encoding) {
    if (this.config.debug) this._lastRequest = req;
    var message = this.config.scheme.buildMessageToSign.call(this, req, encoding || {});
    req.original.headers['x-auth-signedheaders'] = req.signedHeaders.join(';');
    return this._hmac(message, credentials.secret, encoding.output || this.config.signatureEncoding, encoding.input);
  }
