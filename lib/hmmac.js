...
Hmmac.prototype.sign = function(req, credentials, returnSignature, encoding) {
  if (typeof returnSignature === 'object') {
    encoding = returnSignature;
  ...
  var signature = this._sign(req, credentials, encoding)
...
Hmmac.prototype._sign = function(req, credentials, encoding) {
  encoding = encoding || { output: '', input: '' };
  this._normalizedHeaders(req);
  ...
  try {
    return this.config.scheme.sign.call(this, req, credentials, encoding);
  }
  catch(err) {
    this.log(err);
...
Hmmac.prototype._hash = function(message, outputEncoding, inputEncoding) {
  var hash = crypto.createHash(this.config.algorithm);
  hash.update(message, inputEncoding || 'utf8');
  return hash.digest(outputEncoding || null);
};

Hmmac.prototype._hmac = function(message, key, outputEncoding, inputEncoding) {
  var hmac = crypto.createHmac(this.config.algorithm, key);
  hmac.update(message, inputEncoding || 'utf8');
  return hmac.digest(outputEncoding || null);
};
...
Hmmac.prototype._normalizedHeaders = function(req) {
  ...
  if (!req.original.headers.hasOwnProperty('host')) {
    req.original.headers['host'] =
      req.original.host + (req.original.hasOwnProperty('port') ? ':' + req.original.port : '');
  }
...
Hmmac.prototype._body = function(req) {
  ...
  // It's a Node request...
  if (req.original.socket) {
    if (req.original._body) {
      body = req.original.body;
    } else {
      body = '';
    }
  }
  else if ('body' in req.original) {
    body = req.original.body || '';
  }
...
Hmmac.prototype._path = function(req) {
  var path;

  if ('originalUrl' in req.original) {
    path = req.original.originalUrl;

  } else if ('path' in req.original) {
    path = req.original.path;

  } else if ('url' in req.original) {
